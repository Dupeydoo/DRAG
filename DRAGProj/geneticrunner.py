...
context = dc.context
"""
context (:obj:`dict`): The datacontext containing the genetic algorithm 
parameters.
"""
...
"""
tournament_size (int): The tournament size parameter used in tournament 
selection.
"""
...
DEFAULT_POP_SIZE = 10
"""
DEFAULT_POP_SIZE (int): A default population size constant for incorrect 
population sizes.
"""
...
def initiliase_population(input_list, genre):
    ...
    if dm.is_even(population_size) and population_size >= 0:
        return pg.generate_population(population_size, copy_ratio, input_list,
                                      genre, time_signature)
    else:
        # If incorrect input is set for whatever reason, assume a default.
        pop_size = DEFAULT_POP_SIZE
        return pg.generate_population(pop_size, copy_ratio, input_list, genre,
                                      time_signature)
...
def process_input(population, bpm, request):
    ...
    Args:
        ...
        bpm (int): A number representing the beats per minute or tempo of the
        track.

        request (:obj:`HTTPRequest`): The request asking for tracks to be
        generated.
    """
...
def perform_genetics(population):
    ...
    Returns:
        new_population (:obj:`list` of :obj:`Track`): The new population of
        tracks.
    """

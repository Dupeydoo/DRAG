...
class TestPopulationGenerator(unittest.TestCase):
    ...
    def test_generate_population(self):
        ...
        """
        self.population = pg.generate_population(self.p_size, 0.4,
            self.input_list, self.genre, self.time_sig)
        correct_population = True if len(self.population) == 10 else False
        self.assertTrue(correct_population,
                        "The population has not been built as expected!")

    def test_populate_copies_length(self):
        """
        Tests the populate_copies function for length results.
        """
        self.population = pg.populate_copies(self.population, self.copy_number,
                                             self.input_list)
    ...
    def test_populate_copies_content(self):
        ...
        self.assertTrue(not False in element_match,
                        "The elements of the copies don't match the input_list!")

    def tearDown(self):
        """
        Run after each test method. Releases the test resources.
        """
        del self.population, self.p_size, self.copy_number, self.input_list, \
            self.genre, self.time_sig
